...
router.get("/more/:gameId/:cursor", async (req, res) => {
  try {
    const moreStreams = await fetch(
      `https://api.twitch.tv/helix/streams?game_id=${req.params.gameId}&after=${req.params.cursor}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${process.env.ACCESS_TOKEN}`,
...
router.get("/token", async (req, res) => {
  try {
    ...
    console.log(err, "could not receive token");
  }
});
